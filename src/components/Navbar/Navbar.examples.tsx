...
export const data: ComponentExamplesData = {
  ...
  examples: [
    ...
    {
      title: 'The shell: a bar above, a rail beside',
      description:
        'Navbar and Drawer are painted from the same surface and the same edge token, so they meet at the corner without a seam. The bar sits above the row that holds the rail, which is what puts the sidebar toggle in the bar and lets the rail collapse under it.',
      code: [
        ...
        '    </Drawer>',
        '    <Box sx={{ flex: 1, p: 3, overflowY: "auto" }}>{children}</Box>',
        '  </Stack>',
        '</Stack>',
      ].join('\n'),
    },
  ...
  dos: [
    ...
    'Pair it with a `permanent` Drawer for the standard shell; the two share their surface and edge tokens',
