...
### 32. Checkbox and Radio — respecified as a CSS box plus a vector mark, with a 2px radius rung that does not exist (added 11 August, nodes 3205:121943 and 3653:28080)
Both selector sets were replaced wholesale. The old sheet (2080:23677) drew each control as a single whole glyph, which is why the components rendered Phosphor's `CheckSquareIcon` / `RadioButtonIcon`. The new sets draw **a box and its mark as separate objects**: the box carries its own fill, 1px stroke and radius per state, while the mark keeps a fixed size. No single icon glyph can express that, so both components now build the box in CSS and render the marks from the exported vectors. The four Phosphor glyphs are gone.

Both sets also gained a `small` boolean — 16px (`Scale/300`) and 12px (`Scale/250`). Mirrored as `size: 'sm' | 'md'` to match `ButtonSize` / `IconButtonSize` / `ChipSize`; nothing was redefined by doing so, since neither control had a size prop before. (`Chip` had to take the same boolean as `dense` in #30 precisely because it already had `sm` and `md`.)

Behaviour stays MUI's throughout, including its selection animation: the marks stay mounted in every state and grow from `scale(0)` over `duration.shortest`, which is what `RadioButtonIcon` does internally. That only works because all of MUI's icon slots are handed the *same* element — handing each slot a different one remounts the mark at full size and the transition never fires. MUI's ripple, its circular hover tint, and its 9px padding are all untouched.

**Implemented:**

- **`unselect` on the checkbox axis is the indeterminate dash, not the unchecked box.** The unchecked state is `enabled`; `unselect` draws a zero-height 8px line and takes the *same* `surface/primary/default` fill as `selected`. So checked and indeterminate differ only by which mark is at full scale, which is how it is built.
- **Every mark is the exported vector, verbatim.** Check `M0.750001 4.35L3.03572 6.75L8.75 0.750001` at stroke 1.5 in a 9.5x7.8375 box, and its 6x4 `sm` twin at stroke 1.2; dash `M0.75 0.75H8.75` and `M0.75 0.75H6.75`; dot as a 10px / 6px circle. The stroke box carries decimals because Figma centres a nominal 8x6 leaf and lets the round caps overhang it.
- **`border/primary/1` and `border/primary/2` map exactly** onto `border.primary.default` (`primary/100`) and `border.primary.defaultHover` (`primary/200`), which is also what they mean semantically: resting stroke and hover stroke.
- **A transparent 1px border stands in for the states Figma draws with no stroke** (all three disabled checkboxes, plus checked and indeterminate). A background paints under a transparent border, so it renders identically while keeping the box exactly 16px in every state instead of shifting by 1px.

**Not implemented — flagged:**

- **`Scale/50` (2px) has no rung on the radius ladder.** `radius` runs 0 / 4 / 8 / 12 / 16 / 24 / full, so the checkbox's corner is a literal (`BOX_RADIUS_PX`). Same shape of gap as #31's 16px padding and #30's raw `6`. **Confirm** whether `radius` should gain a 2px rung — it is the smallest real corner in the system and nothing else currently needs it.
- **Neither set draws a focus-visible state.** MUI leaves SwitchBase without one too, so shipping as drawn would mean a control with no keyboard affordance at all. Both now carry the same 3px `border.primary.focus` ring that `Button` and `IconButton` use. **Confirm** the ring is right for a 16px control; it is nearly as wide as the box.
- **The checkbox has no disabled-unchecked cell.** The set draws `selected-disabled` and `unselect-disabled` but never a plain disabled empty box. Both drawn disabled states are borderless, so borderlessness was extended to the third for within-set consistency — which makes a disabled unchecked checkbox a plain grey square with no mark, reading as filled rather than empty. Note the radio set solves the analogous case the other way, giving its unselected `disabled` a border. **Confirm** which is intended; a border would make it read as empty.
- **The checkbox has no hover for a filled box.** `hover` exists only for the unchecked state. Since MUI's tint is a faint circle in the padding rather than on the box, a checked box would have had almost no pointer feedback, so checked-hover uses `surface.primary.defaultHover` — the system's own hover partner for `surface.primary.default`, the pairing `Button` already uses for a contained fill, not a new value. **Confirm.**
- **`icon/primary/4` (`primary/400`, `#5f6aea`) has no semantically-named home.** It is the radio dot. The only `icon.*` slot holding `primary/400` in light mode is `onColorHover`, which is the wrong name for this. Drawn from `icon.primary.accent` instead (`primary/500` light, `primary/400` dark) — one rung darker than specified, so higher contrast, and it makes the dot match the checkbox's checked fill exactly. **Confirm** whether the dot was meant to be a rung lighter than the checkbox fill, or whether the numbered `icon/primary/*` rungs should be exposed as tokens.
- **The disabled marks are specified two rungs apart across the two sets.** The disabled checkbox mark is `#cccac6` (`grey/300`, whose semantic home is `border/disabled/default`) while the disabled radio dot is `#aeaba4` (`grey/500`, `icon/disabled/default`). Both are drawn from `icon.disabled.default` here, because `border.disabled.default`'s dark value is `grey/900` — exactly the disabled surface's dark value, so a `grey/300`-derived mark would vanish in dark mode. The result is a slightly darker disabled check than drawn. **Confirm** the two sets should agree.
- **The disabled radio's border is invisible in dark mode.** `border.disabled.default` dark and `surface.disabled.default` dark are both `grey/900`, so the stroke the set draws around a disabled radio has nothing to separate it from its own fill. Verified in the browser: both resolve to `rgb(36, 36, 33)`. Same failure mode as #31's card border and #26's divider. **Confirm** the dark pairing.
- **Hover and selected are the same picture on the radio.** Both take `surface/layers/card-2` with a `border/primary/2` stroke, so a hovered unselected radio is pixel-identical to a selected one minus the dot. Carried as drawn, and the docs now say so, but it means the dot is the only unambiguous selection signal. **Confirm.**
- **The two controls rest on different surfaces.** The unchecked checkbox is `surface/layers/card-1` (`#fafaf9`); the unselected radio is `surface/layers/card-2` (`#f5f5f3`). On a form holding both, the two sit a rung apart for no evident reason. **Confirm** whether one of them is a slip.
- **The `sm` marks are not the same weight.** The small check thins to stroke 1.2 while the small dash stays at 1.5. Carried as drawn. **Confirm.**
- **`foundations/white` is a raw primitive with no mode pair.** It is the mark on a filled checkbox. The repo exposes no white; the nearest semantic token is `icon.default.headingOnColor` (`grey/25`), whose dark value inverts to near-black — correct for a glyph on a surface that turns light in dark mode, but this one does not, since the checked box stays a saturated `primary/600`. Pinned to the light value in both schemes. **Confirm** the intended token.

**Verified unchanged, so nothing was assumed:** the 1px stroke on every cell of both sets; the 16 / 12px box edges; the checkbox's `Scale/50` corner and the radio's fully-round one (`Scale/400` on a 16px box); `surface/primary/default` shared by `selected` and `unselect`; `surface/primary/subtle` with `border/primary/2` for the checkbox hover; `surface/disabled/default` on every disabled cell of both sets; the dot's 2px,2px placement inside the border, which a centred 10px circle in a 14px content box reproduces exactly; and the mark geometry listed above.
